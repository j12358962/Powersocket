...
def labeling(diff, engine):
    ...
    # 從my_device，把既有電器取出來###################################
    if incr == 1:
        # 這裡where條件把背景noise去掉,以及早就是開的狀態者去掉
        sql_db = "select * from my_device where ID != 1 and status != 1"
    else:
        # 這裡where條件把背景noise去掉,以及早就是關的狀態者去掉
        sql_db = "select * from my_device where ID != 1 and status != 0"
    ...
    for i in range(0, len(diff), 1):
        ...
        for j in range(0, len(np_db), 1):
            ...
            if abs(abs(diff[i, 1]) - np_db[j, 1]) < 0.03:  # 0.04
    ...
    if len(np_db) == 0:  # new system no appliance in db
        ...
        belong = 0
        for r in set(record):
            print(r, ":", list(record).count(r))  # test
        if max(set(record), key=record.count) != 0:  # 最多的非0(非未知的意思)
            print("\n有max不是0,是", max(set(record), key=record.count))
...
def centroid_cal(sums):
    ...
    centroid = np.mean(sums, axis=0)
    radius = 0
    for i in range(0, len(sums), 1):
        dist = np.linalg.norm(sums[i, :] - centroid[:])
        radius = radius + dist
    radius = radius / len(sums)
